...
export default function SeriesClient() {
  ...
  return (
    <div className="pt-20 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-8">
      ...
      {/* BARRA DE COMPLETUDE DA RUN & ALERTA DE LACUNAS */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 p-6 rounded-3xl bg-bg-surface border border-border-default shadow-card space-y-4">
          ...
          <div className="w-full h-3 rounded-full bg-bg-elevated overflow-hidden border border-border-default">
            <div
              className="h-full bg-gradient-to-r from-brand-primary to-emerald-400 transition-all duration-700 rounded-full"
          ...
          <div className="flex items-center justify-between text-xs text-text-secondary pt-1">
            <span className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-500" /> {ownedCount} Adquiridas
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full bg-amber-500" /> {missingIssues.length} Faltantes
            </span>
          </div>
        </div>

        {/* Card de Lacunas (Gap Detection) */}
        <div className="p-6 rounded-3xl bg-amber-500/10 border border-amber-500/20 flex flex-col justify-between space-y-3">
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-amber-500">
              <AlertCircle className="w-5 h-5" />
              <h3 className="text-xs font-black uppercase tracking-wider">Lacunas Detectadas</h3>
            </div>
            <p className="text-xs text-text-secondary leading-relaxed">
              Você possui <strong>{ownedCount} de {totalIssues}</strong> edições. Faltam {missingIssues.length} volumes para completar a fase inteira!
            </p>
          </div>
          ...
          {missingIssues.length > 0 && (
            <div className="pt-2">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-full py-2.5 px-4 rounded-xl bg-amber-500 text-black text-xs font-black flex items-center justify-center gap-2 hover:bg-amber-400 transition-all shadow-sm"
              >
                Completar Coleção na Panini
      ...
      {/* Grid de Edições da Série */}
      <div className="space-y-4">
        <h2 className="text-lg font-black text-text-primary">Linha do Tempo dos Volumes</h2>
            ...
            return (
              <div
                ...
                className={`group flex flex-col justify-between p-3 rounded-2xl border transition-all duration-200 ${
                  isOwned
                    ? "bg-bg-surface border-border-default shadow-card hover:border-brand-primary/50"
                    : "bg-bg-surface/50 border-dashed border-amber-500/40 opacity-85 hover:opacity-100"
                }`}
              >
                <div className="space-y-2.5">
                  <div className="relative aspect-[2/3] rounded-xl overflow-hidden bg-bg-elevated">
                    ...
                    <div className="absolute top-2 left-2">
                      <span className="px-2 py-0.5 rounded-full bg-black/70 backdrop-blur-md text-[10px] font-black text-white">
                        Vol. {issue.volume}
                      </span>
                    </div>
                ...
                <button
                  ...
                  className={`w-full mt-2.5 py-1.5 px-2 rounded-lg text-[11px] font-bold border transition-all ${
                    issue.status === "falta"
